...
<?if (!function_exists('getPropValue'))
{
	function getPropValue(&$item, $propertyCode)
	{   

		$res = CIBlockElement::GetProperty($item['IBLOCK_ID'], $item['ID'], array("sort" => "asc"), Array("CODE"=>$propertyCode));
         
     	while ($ob = $res->GetNext()) { 

     		$item[$propertyCode] = $ob;
...
if (!function_exists('getInputFilterChunk'))
{
function getInputFilterChunk ($value) {
   
	$chunk = '<div class="form_control"> 
			  <input type="checkbox" data-id="'.$value['ID'].'" name="filter" value="'.$value['NAME'].'" id="filter_'.$value['ID'].'"	'.$value['checked'].'/> 
